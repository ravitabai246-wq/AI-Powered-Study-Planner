module.exports = async function handler(req, res) {
  ...
  for (var i = 0; i < subjects.length; i++) {
    subjectsList += '- ' + subjects[i].name + ': needs ' + subjects[i].hours + ' hours, priority: ' + subjects[i].priority + '\n';
  }

  var systemPrompt = 'You are an expert academic study planner. Your job is to create a clear, day-by-day study schedule for a student based on the subjects/topics they provide, their priorities, the hours needed for each, the number of days available, and the hours they can study per day. ' +
    'Rules you must follow: ' +
    '1. Prioritize High priority subjects earlier and give them more time. ' +
    '2. Never schedule more total hours in a day than the students stated hours-per-day limit. ' +
  ...
  var userPrompt = 'Here are my subjects/topics:\n' + subjectsList +
  ...
  try {
    ...
    var plan = data.candidates[0].content.parts[0].text;
